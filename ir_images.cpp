...
void trackObject(IplImage* imgThresh) {
  ...
  while (contour) {
    ...
    // approx area of target up close(~1-2ft) is 18000, ~5-6ft is 5175   ON
    // webcam if there are 4 vertices  in the contour and the area of the rect is
    ...
    if (result->total == 4 && fabs(cvContourArea(result, CV_WHOLE_SEQ)) > 100) {
      ...
      bool target = 0;  // target -3 is ignorable, target 1 is long side first
                        // (R), target 2 is short side first (L)
      // origin (,0,0) is TOP left.

      ////CONFIRM TARGET SIDE BY Y VALUES

      // if point 0 and 1 are both higher than pts 2 and 3, then Left T )
      if ((pt[0]->y < pt[3]->y) && (pt[0]->y < pt[2]->y) &&
          (pt[1]->y < pt[3]->y) && (pt[1]->y < pt[2]->y)) {
        ...
        b = sqrt((pt[2]->x - pt[1]->x) ^ 2 + (pt[2]->y - pt[1]->y) ^ 2);  // 2,
                                                                          // 1
        c = sqrt((pt[2]->x - pt[3]->x) ^ 2 + (pt[2]->y - pt[3]->y) ^ 2);
        d = sqrt((pt[3]->x - pt[0]->x) ^ 2 + (pt[3]->y - pt[0]->y) ^ 2);  // 3,0
        cout << "point 1 (inner top) of left target: " << pt[1]->x << ", "
             << pt[1]->y << "\n";

      } else if ((pt[3]->y < pt[2]->y) && (pt[0]->y < pt[1]->y) &&
                 (pt[3]->y < pt[1]->y) && (pt[0]->y < pt[2]->y)) {
        ...
        b = sqrt((pt[0]->x - pt[1]->x) ^ 2 + (pt[0]->y - pt[1]->y) ^ 2);  // 1,
                                                                          // 0
        c = sqrt((pt[1]->x - pt[2]->x) ^ 2 + (pt[1]->y - pt[2]->y) ^ 2);  // 21
        d = sqrt((pt[3]->x - pt[2]->x) ^ 2 + (pt[3]->y - pt[2]->y) ^ 2);  // 2, 3

        cout << "point 0 (inner top) of right target: " << pt[0]->x << ", "
             << pt[0]->y << "\n";
      ...
      /*
      if ( pt[3]->x < pt[1]->x && pt[3]->x < pt[2]->x && pt[0]->x < pt[1]->x &&
      pt[0]->x < pt[2]->x) { target = 2; //left target (confirmed by x)
      }
      else if ( pt[3]->x < pt[0]->x && pt[3]->x < pt[1]->x && pt[2]->x <
      pt[1]->x && pt[2]->x < pt[0]->x ) { target = 1; //right target confirmed
      by x
      }
      else { target = -3; }
      */

      // cout << "lengths dist_a, dist_b, dist_c, & dist_d: " << dist_a << " , "
      // << dist_b << " , "  << dist_c << " , "  << dist_d << "\n";

      bool similarSides = false;
      bool similarSides2 = false;

      // CHECK for matching sides (if a is close enough to its counterpart
      // across (c)
      ...
      if (target == 1) {  // if right target (a & c are long sides) (0 is top right)
        ...
        if (dist_a > (dist_b + min) && dist_a > (dist_d + min) &&
            dist_c > (dist_b + min) && dist_c > (dist_d + min) &&
            pt[0]->y < pt[1]->y - min && pt[3]->y < pt[2]->y - min) {
      ...
      if (target == 2) {  // target is left (b & d are long sides) (0 it top left)
        ...
        if (dist_b > (dist_a + min) && dist_b > (dist_c + min) &&
            dist_d > (dist_a + min) && dist_d > (dist_c + min) &&
            pt[0]->y < pt[3]->y - min && pt[1]->y < pt[2]->y - min) {
      ...
      } else if (((g / e) > 2.74) && ((g / e) < 2.76) && ((e / g) > 0.34) &&
                 ((e / g) < 0.38)) {
      ...
      int dist1 = (pt[1]->x - pt[0]->x) * (pt[1]->x - pt[0]->x) +
                  (pt[1]->y - pt[0]->y) * (pt[1]->y - pt[0]->y);
      int dist2 = (pt[2]->x - pt[1]->x) * (pt[2]->x - pt[1]->x) +
                  (pt[2]->y - pt[1]->y) * (pt[2]->y - pt[1]->y);
      ...
      if (target == 1 || target == 2) { //if target is Right side(1) or left
      side (2) (skip if neither)
      ...
      */
      ...
      if ((target == 1 || target == 2) && 
        similarSides && similarSides2 &&
          pt[0]->y > imgTracking->height / 4) {
        int h = pt[2]->y - pt[0]->y;

        // cout << "REAL target = " << realTarget << "\n";
        // print coords
        /*cout << "Point 0 coords (x, y):  " << pt[0]->x << " , " << pt[0]->y <<
        "\n"; cout << "Point 1 coords (x, y):  " << pt[1]->x << " , " <<
        pt[1]->y << "\n"; cout << "Point 2 coords (x, y):  " << pt[2]->x << " ,
        " << pt[2]->y << "\n"; cout << "Point 3 coords (x, y):  " << pt[3]->x <<
        " , " << pt[3]->y << "\n";
        */

        // drawing lines around the quadrilateral //the highest point vrtically
        // is first.
        ...
        //NetworkTable::GetTable("Vision"); 	 visionTable->PutNumber("Distance",
        //distanceToTarget); double  distanceFromCenterToVisionTarget =
        // focalLengthCenter * lengthCenter / distanceToTarget;
        ...
        if (h > 0) {
          cout << " and center = " << distanceToCenter
               << " and taret = " << target << " and realtarget " << realTarget
               << "\n";
        }
...
int main() {
  ...
  while (true) {
    ...
    cvThreshold(imgGrayScale, imgGrayScale, 160, 255,
                CV_THRESH_BINARY_INV);  // threshold was 150 (of 255) //180
